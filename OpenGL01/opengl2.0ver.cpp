...
int main()
{
    ...
    //链接顶点属性
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)0);//GL_FALSE表示不被标准化
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(3 * sizeof(float)));//GL_FALSE表示不被标准化
    glEnableVertexAttribArray(1);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        const glm::vec3 &objectColor = glm::vec3(0.9f, 0.8f, 0.8f);//设置物体本体颜色
        glUniform3fv(glGetUniformLocation(myShader.ID, "objectColor"), 1, &objectColor[0]);//传入片段着色器中的全局变量
        myShader.setVec3("lightPos", lightPos);//传入片段着色器中的光源位置
        myShader.setVec3("viewPos", cameraPos);//传入观察者位置
