...
void main(){
    ...
    vec3 N = normalize(Normal1);//将传进来的世界坐标法线标准化
    ...
    for(int i=0;i<L_Num;i++)
    {
        ...
        Lcol += (KD*texture(head, TexCoord).rgb/PI + specular) * radiance * NdotL; //* radiance
    }

	vec3 albedo = texture(head, TexCoord).rgb;
